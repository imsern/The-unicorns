...
function onOff3x3(x) {
  if (lys2[x] == 0) {
    document.getElementById(rk[x]).style.backgroundColor = "rgb(15, 214, 15)";
    lys2[x] = 1;
  } else {
...
function reset() {
    ...
    for (i in rc){
        ...
        document.getElementById(rc[i]).style.boxShadow = "none";
        lys[i]=0;
    }
    for (i in rk){
        document.getElementById(rk[i]).style.backgroundColor="rgb(255, 5, 5)";
        document.getElementById(rk[i]).style.boxShadow = "none";
        lys2[i]=0;
    } 
}
